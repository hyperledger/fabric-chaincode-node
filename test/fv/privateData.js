...
describe('Chaincode privateData', () => {
    ...
    before(async function () {
        ...
        return utils.installAndInstantiate(suite, { transient: true });
    });
    ...
    it('should write and read an asset with private data', async function () {
        ...
        await utils.invoke(
            suite,
            'privateDataContract:createAsset',
            ['1'],
            `{"privateValue":"${privateData}"}`
        );
        const payload = await utils.query(
            suite,
            'privateDataContract:readAsset',
            ['1']
        );
    ...
    it('should update an asset with private data', async function () {
        ...
        await utils.invoke(
            suite,
            'privateDataContract:updateAsset',
            ['2'],
            `{"privateValue":"${privateData}"}`
        );
        const payload = await utils.query(
    ...
    it('should delete an asset with private data', async function () {
        ...
        payload = await utils.query(suite, 'privateDataContract:assetExists', [
            '3',
        ]);
    ...
    it('should call purge private data without failing', async function () {
        ...
        expect(payload).to.eql('{"privateValue":"privateData"}');
        console.log(
            await utils.invoke(suite, 'privateDataContract:purgeAsset', ['4'])
        );
        payload = await utils.query(suite, 'privateDataContract:assetExists', [
            '4',
        ]);
