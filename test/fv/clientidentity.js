...
describe('Chaincode clientidentity', () => {
    ...
    before(async function () {
        ...
        return utils.installAndInstantiate(suite, {
            instantiateFunc: 'org.mynamespace.clientidentity:instantiate',
        });
    });
    ...
    it('should create an instance of the client identity class', async function () {
        ...
        const payload = JSON.parse(
            await utils.query(
                suite,
                'org.mynamespace.clientidentity:clientIdentityInstance',
                []
            )
        );
        expect(payload.mspId).to.equal('Org2MSP', 'Test mspId value');
        expect(payload.id).to.equal(
            'x509::/C=US/ST=North Carolina/O=Hyperledger/OU=client/CN=buyer::/C=UK/ST=Hampshire/L=Hursley/O=org2.example.com/CN=ca.org2.example.com',
            'Test getID()'
        );
    });
    ...
    it('should be able to check the peer MSPID', async function () {
        ...
        const payload = JSON.parse(
            await utils.query(
                suite,
                'org.mynamespace.clientidentity:localMspID',
                []
            )
        );
