...
describe('Typescript chaincode', () => {
    ...
    describe('Scenario', () => {
        it('should write an asset', async function () {
            this.timeout(LONG_STEP);
            await utils.invoke(suite, 'TestContract:createAsset', [
                'GLD',
                'GOLD_BAR',
                '100',
                'EXTRA_ID',
            ...
            const payload = JSON.parse(
                await utils.query(suite, 'TestContract:getAsset', ['GLD'])
            );
            expect(payload).to.eql({
                id: 'GLD',
                name: 'GOLD_BAR',
                value: 100,
                extra: { id: 'EXTRA_ID', value: 50 },
            });
        });

        it('should update an asset', async function () {
            this.timeout(SHORT_STEP);
            await utils.invoke(suite, 'TestContract:updateAsset', [
                JSON.stringify({
                    id: 'GLD',
                    name: 'GOLD_BAR',
                    value: 200,
                    extra: { id: 'EXTRA_ID', value: 100 },
                }),
            ]);
            const payload = JSON.parse(
                await utils.query(suite, 'TestContract:getAsset', ['GLD'])
        ...
        it('should handle the getMetadata', async function () {
            this.timeout(SHORT_STEP);
            const payload = JSON.parse(
                await utils.query(suite, 'org.hyperledger.fabric:GetMetadata')
            );

            const schema = fs.readFileSync(
                path.join(
                    __dirname,
                    '../../apis/fabric-contract-api/schema/contract-schema.json'
                )
            );
            ...
            if (!ajv.validate(JSON.parse(schema), payload)) {
                throw new Error(
                    'Expected generated metadata to match the schema'
                );
            }
            ...
            const expectedMetadata = fs.readFileSync(
                path.join(
                    __dirname,
                    '../chaincodes/annotations/src/test_contract/expected-metadata.json'
                )
            );
