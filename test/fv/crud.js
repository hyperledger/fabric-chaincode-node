...
describe('Chaincode CRUD', () => {
    ...
    before(async function () {
        ...
        return utils.installAndInstantiate(suite, {
            instantiateFunc: 'org.mynamespace.crud:instantiate',
        });
    });
    ...
    describe('Get', () => {
        ...
        it('should get multiple keys concurrently', async function () {
            ...
            const payload = await utils.query(
                suite,
                'org.mynamespace.crud:getKeysConcurrently',
                ['key1', 'key2', 'key3']
            );
            expect(JSON.parse(payload)).to.deep.equal([
                'value1',
        ...
        it('should get a single composite key value', async function () {
            ...
            const payload = await utils.query(
                suite,
                'org.mynamespace.crud:getCompositeKey',
                ['ann', 'black']
            );
        ...
        it('should return a list of results from a partial composite key using the old iterator style', async function () {
            ...
            const payload = await utils.query(
                suite,
                'org.mynamespace.crud:getPartialCompositeKey',
                ['ann']
            );
            expect(JSON.parse(payload)).to.deep.equal([
                'annblack',
        ...
        it('should return a list of results from a partial composite key using the new iterator style', async function () {
            ...
            const payload = await utils.query(
                suite,
                'org.mynamespace.crud:getPartialCompositeKeyUsingAsyncIterator',
                ['ann']
            );
            expect(JSON.parse(payload)).to.deep.equal([
                'annblack',
        ...
        it('should return all keys between key1 and key3 using the old iterator style', async function () {
            ...
            const payload = await utils.query(
                suite,
                'org.mynamespace.crud:getKeysByRange',
                ['key1', 'key4']
            );
            expect(JSON.parse(payload)).to.deep.equal([
                'value1',
        ...
        it('should return all keys between key1 and key3 using the new iterator style', async function () {
            ...
            const payload = await utils.query(
                suite,
                'org.mynamespace.crud:getKeysByRangeUsingAsyncIterator',
                ['key1', 'key4']
            );
            expect(JSON.parse(payload)).to.deep.equal([
                'value1',
        ...
        it('should get the history for a key using the old and new iterator style', async function () {
            ...
            await utils.invoke(suite, 'org.mynamespace.crud:putKey', [
                'key1',
                'newValue1',
            ]);
            await utils.invoke(suite, 'org.mynamespace.crud:putKey', [
            ...
            let payload = await utils.query(
                suite,
                'org.mynamespace.crud:getHistoryForKey',
                ['key1']
            );
            expect(JSON.parse(payload)).to.deep.equal([
                'value1',
            ...
            payload = await utils.query(
                suite,
                'org.mynamespace.crud:getHistoryForKeyUsingAsyncIterator',
                ['key1']
            );
            expect(JSON.parse(payload)).to.deep.equal([
                'value1',
        ...
        it('should get a query result with pagination without setting a bookmark using the old iterator style', async function () {
            ...
            const payload = JSON.parse(
                await utils.query(
                    suite,
                    'org.mynamespace.crud:getQueryResultWithPagination',
                    []
                )
            );
        ...
        it('should get a query result with pagination without setting a bookmark using the new iterator style', async function () {
            ...
            const payload = JSON.parse(
                await utils.query(
                    suite,
                    'org.mynamespace.crud:getQueryResultWithPaginationUsingAsyncIterator',
                    []
                )
            );
            expect(payload.results1.length).to.equal(2, 'Should return 2 keys');
        ...
        it('should return a paginated list using the old iterator style', async function () {
            ...
            const payload1 = await utils.query(
                suite,
                'org.mynamespace.crud:getStateByRangeWithPagination',
                ['key1', 'key4', 2, 'key2']
            );
            const payload2 = await utils.query(
                suite,
                'org.mynamespace.crud:getStateByRangeWithPagination',
                ['key1', 'key4', 3, '']
            );
            expect(JSON.parse(payload1)).to.deep.equal(['value2', 'value3']);
            expect(JSON.parse(payload2)).to.deep.equal([
                'value1',
                'value2',
                'value3',
            ]);
        ...
        it('should return a paginated list using the new iterator style', async function () {
            ...
            const payload1 = await utils.query(
                suite,
                'org.mynamespace.crud:getStateByRangeWithPaginationUsingAsyncIterator',
                ['key1', 'key4', 2, 'key2']
            );
            const payload2 = await utils.query(
                suite,
        ...
        it('should return a state from a partial composite key using the old iterator style', async function () {
            ...
            const payload = await utils.query(
                suite,
                'org.mynamespace.crud:getStateByPartialCompositeKey',
                ['name~color', 'ann']
            );
            expect(JSON.parse(payload)).to.deep.equal([
                'annblack',
        ...
        it('should return a state from a partial composite key using the new iterator style', async function () {
            ...
            const payload = await utils.query(
                suite,
                'org.mynamespace.crud:getStateByPartialCompositeKeyUsingAsyncIterator',
                ['name~color', 'ann']
            );
            expect(JSON.parse(payload)).to.deep.equal([
                'annblack',
        ...
        it('should return a paginated list from partial composte key using the old iterator style', async function () {
            ...
            const payload = await utils.query(
                suite,
                'org.mynamespace.crud:getStateByPartialCompositeKeyWithPagination',
                ['name~color', 1]
            );
        ...
        it('should return a paginated list from partial composte key using the new iterator style', async function () {
            ...
            const payload = await utils.query(
                suite,
                'org.mynamespace.crud:getStateByPartialCompositeKeyWithPaginationUsingAsyncIterator',
                ['name~color', 1]
            );
            expect(JSON.parse(payload)).to.deep.equal(['annblack']);
        });
    ...
    describe('Put', () => {
        ...
        it('should add a composite key', async function () {
            this.timeout(SHORT_STEP);
            await utils.invoke(suite, 'org.mynamespace.crud:putCompositeKey', [
                'tim',
                'green',
                'newCompositeValue',
            ]);
    ...
    describe('Delete', () => {
        it('should delete a key', async function () {
            this.timeout(SHORT_STEP);
            await utils.invoke(suite, 'org.mynamespace.crud:deleteKey', [
                'key2',
            ]);
            const payload = await utils.query(
                suite,
        ...
        it('should delete a composite key', async function () {
            ...
            await utils.invoke(
                suite,
                'org.mynamespace.crud:deleteCompositeKey',
                ['tim', 'green']
            );
            const payload = await utils.query(
                suite,
    ...
    describe('Split', () => {
        it('should split the composite key', async function () {
            ...
            const payload = JSON.parse(
                await utils.query(
                    suite,
                    'org.mynamespace.crud:splitCompositeKey',
                    ['beth']
                )
            );
            expect(payload.results.length).to.equal(
                3,
                'Should return 3 composite key matching the name "beth"'
            );

            expect(payload.key1.objectType).to.equal(
                'name~color',
                '"objectType" value of the returned composite key should be "name~color"'
            );
            expect(payload.key1.attributes.length).to.equal(
                2,
                '"attributes" value of the returned composite key should be array of size 2'
            );
            expect(payload.key1.attributes[0]).to.equal(
                'beth',
                '1st attribute value of the returned composite key should be "beth"'
            );
            expect(payload.key1.attributes[1]).to.equal(
                'black',
                '2nd attribute value of the returned composite key should be "black"'
            );

            expect(payload.key2.objectType).to.equal(
                'name~color',
                '"objectType" value of the returned composite key should be "name~color"'
            );
            expect(payload.key2.attributes.length).to.equal(
                2,
                '"attributes" value of the returned composite key should be array of size 2'
            );
            expect(payload.key2.attributes[0]).to.equal(
                'beth',
                '1st attribute value of the returned composite key should be "beth"'
            );
            expect(payload.key2.attributes[1]).to.equal(
                'red',
                '2nd attribute value of the returned composite key should be "red"'
            );

            expect(payload.key3.objectType).to.equal(
                'name~color',
                '"objectType" value of the returned composite key should be "name~color"'
            );
            expect(payload.key3.attributes.length).to.equal(
                2,
                '"attributes" value of the returned composite key should be array of size 2'
            );
            expect(payload.key3.attributes[0]).to.equal(
                'beth',
                '1st attribute value of the returned composite key should be "beth"'
            );
            expect(payload.key3.attributes[1]).to.equal(
                'yellow',
                '2nd attribute value of the returned composite key should be "yellow"'
            );
        });
    });
