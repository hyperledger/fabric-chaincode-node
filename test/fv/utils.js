...
async function install(
    ...
    try {
        ...
        const collectionConfig = transient
            ? `-cccg ${CCDir}/collection-config/collection.json`
            : '';
        const cmd = `cd ${networkScriptDir} && ./network.sh deployCC -ccn ${ccName} -ccp ${CCDir} -ccl ${ccType} ${collectionConfig} -ccep "OR('Org1MSP.peer','Org2MSP.peer')"`;
...
async function instantiate(ccName, func, args) {
    ...
    if (func !== undefined) {
        const initCmd =
            ...
            `export CORE_PEER_ADDRESS=localhost:7051 && peer chaincode invoke ${getTLSArgs()} -o localhost:7050 -C mychannel -n ${ccName} -c '${printArgs(
                func,
                args
            )}' --waitForEvent`;
...
async function invoke(ccName, func, args, transient) {
    ...
    if (transient) {
        cmd += `peer chaincode invoke ${getTLSArgs()} -o localhost:7050 -C mychannel -n ${ccName} -c '${printArgs(
            func,
            args
        )}' --transient '${transient}' --waitForEvent --waitForEventTimeout 200s 2>&1`;
    } else {
        cmd += `peer chaincode invoke ${getTLSArgs()} -o localhost:7050 -C mychannel -n ${ccName} -c '${printArgs(
            func,
            args
        )}' --waitForEvent --waitForEventTimeout 100s 2>&1`;
    }
...
async function query(ccName, func, args, transient) {
    ...
    if (transient) {
        cmd += `peer chaincode query ${getTLSArgs()} -C mychannel -n ${ccName} -c '${printArgs(
            func,
            args
        )}' --transient '${transient}' 2>&1`;
    } else {
        cmd += `peer chaincode query ${getTLSArgs()} -C mychannel -n ${ccName} -c '${printArgs(
            func,
            args
    ...
    return stdout
        .trim()
        .replace(/^"(.*)"$/, '$1')
        .replace(/\\"/g, '"'); // remove surrounding quotes and unescape
}

async function installAndInstantiate(
    ccName,
    options = {
        instantiateFunc: undefined,
        instantiateArgs: undefined,
        transient: false,
        ccType: 'javascript',
    }
) {
    await install(ccName, {
        ...
        ccType: options.ccType ?? 'javascript',
    });
    return instantiate(
        ccName,
        options.instantiateFunc ?? undefined,
        options.instantiateArgs ?? undefined
    );
}
