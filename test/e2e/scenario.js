...
const queryFunctions = async () => {
    ...
    if (JSON.stringify(metadata) !== expectedMetadata) {
        throw new Error(
            `Expected query response to equal ${expectedMetadata} \ninstead recieved: \n${JSON.stringify(
                metadata
            )}`
        );
    }
...
const findPackageId = (queryOutput, label) => {
    ...
    const cc = output.installed_chaincodes.filter(
        (chaincode) => chaincode.label === label
    );
    if (cc.length !== 1) {
