...
const invokeFunctions = async () => {
    ...
    const args = util.format('docker exec org1_cli peer chaincode invoke %s -C %s -n %s -c %s --waitForEvent',
        ...
        'server',
        '\'{"Args":["putValue","\'42\'"]}\'');
            let cmd1=`docker exec peer0.org1.example.com sh -c "ls -lartR /var/hyperledger/production/externalbuilder"`
            let cmd2=`docker exec peer0.org1.example.com sh -c "ls -lartR /tmp"`
            let cmd3=`docker exec peer0.org2.example.com sh -c "ls -lartR /var/hyperledger/production/externalbuilder"`
            let cmd4=`docker exec peer0.org2.example.com sh -c "ls -lartR /tmp"`

        try {
            await runcmds([args]);
            console.log(packageId)
            await runcmds([cmd1,cmd2,cmd3,cmd4])
        } catch (e){
            await runcmds([cmd1,cmd2,cmd3,cmd4])
            throw e;
        }
