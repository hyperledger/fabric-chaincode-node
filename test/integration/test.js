...
const Chaincode = class {
    ...
    async test3(stub, args) {
        const iterator = await stub.getStateByRange('key2', 'key6');
    ...
    test9(stub, args) {
        ...
        const p1 = stub.putState(key1, 'dummyValue3')
			.then((res) => {
    ...
    assert.fail('Failed to put a state using composite key ' + key1);
        ...
        const p2 = stub.putState(key2, 'dummyValue4')
			.then((res) => {
    ...
    assert.fail('Failed to put a state using composite key ' + key2);
        ...
        const p3 = stub.putState(key3, 'dummyValue5')
			.then((res) => {
    ...
    assert.fail('Failed to put a state using composite key ' + key3);
        ...
        const p4 = stub.putState(key4, 'dummyValue6')
			.then((res) => {
    ...
    async test10(stub, args) {
        const iterator = await stub.getStateByPartialCompositeKey('color~name', ['blue']);
        const results = await getAllResults(iterator, true /* get keys instead of values */);
    ...
    async test14(stub, args) {
        ...
        const stateRaw = await stub.getState(args[0]);
    ...
    async test15(stub, args) {
        let error;
        try {
            const results = await stub.invokeChaincode('mycc2', ['getKey']);
    ...
    async test19(stub, args) {
        ...
        const response = await stub.getStateByPartialCompositeKeyWithPagination('color~name', [], 3, '');
        const expectedResponse = [key1, key2, key3];
        await checkPagedResponse(response, 3, 3, expectedResponse);
    }
